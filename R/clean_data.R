...
clean_data <- function(dat, min_set_size){
  ...
  if (any(duplicated(dat))) {
    message(stringr::str_wrap(crayon::yellow("ATTENTION: Found duplicated rows
                                             in provided data, removing
                                             duplicates.")))
    dat <- unique(dat)
  ...
  if (!missing(min_set_size)) {
    if (min_set_size <= 0){
      stop(stringr::str_wrap(paste0("You provided ", min_set_size,
                                    " as 'min_set_size'.
                                    Parameter 'min_set_size' should be a
                                    positive number.")))
    } else {
      message(stringr::str_wrap(paste0("Total number of sets in the dataset
                                       is ", length(unique(dat$set.)), ".")))
      ...
      message(paste0("You provided ", min_set_size,
...
crayon::yellow(paste0("ATTENTION: Removed sets where the number of genes is
    ...
    message(stringr::str_wrap("Parameter 'min_set_size' is not provided. No gene
