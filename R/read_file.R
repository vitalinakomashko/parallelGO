...
read_file <- function(path, col.names, delim){
  if(missing(path)){
    ...
    if(!file.exists(path)){
      ...
      message(stringr::str_wrap(crayon::yellow("ATTENTION: For the file to be 
processed correctly ensure that the gene identifiers are in the first column and the 
gene set labels are in the second column.")))
      if(missing(col.names)){
        stop("Please provide TRUE (present) or FALSE (absent) for the 'col.names' parameter.")
      }else{
        col.names <- col.names
      }
      if(missing(delim)){
      ...
      dat <- read.delim(file = path, header = col.names, sep = delim, 
                        stringsAsFactors = FALSE,
                        colClasses = "character",
                        row.names = NULL)
      message(stringr::str_wrap(crayon::green(paste0("SUCCESS: Read the file '", 
                                                     path, "' with ", nrow(dat), 
                                                     " and ", ncol(dat), 
                                                     " columns.\n"))))
      if(ncol(dat) > 2){
        message(stringr::str_wrap(crayon::yellow("ATTENTION: found more than two columns in the file,
                                  retaining only the first two.")))
        dat <- dat[, 1:2]
      }
      colnames(dat) <- c("id", "set.label")
      return(dat)
    }
  }
}